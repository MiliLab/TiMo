...
class TiMo(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                dim=embed_dims[i], sa_num_heads=sa_num_heads[i], mlp_ratio=mlp_ratios[i], qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                use_layerscale=use_layerscale,
                layerscale_value=layerscale_value,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer,
                local=local[i])
                for j in range(depths[i])])#v1:local=0 if i==2 and j%2!=0 else local[i]
        ...
        # self.patch_embed_1 = OverlapPatchEmbed(img_size=self.img_size,
        #                                  patch_size=3,
        #                                  stride=2,
        #                                  in_chans=self.embed_dim[0],
    ...
    def forward_features(self, x):
        ...
        return output[-1]#B*T, C1, H/32,W/32

    def forward(self, x,ts,returntp=False):
        B,T,C,H,W=x.shape
